...
func main() {
	// Keep-Alive 설정으로 연결 생성 (스텁과 채널 재사용)
	conn, err := grpc.Dial("localhost:8080",
		grpc.WithInsecure(), // HTTPS vs HTTP
		grpc.WithKeepaliveParams(keepalive.ClientParameters{
			Time:                60 * time.Second, // 1분마다 핑
			Timeout:             5 * time.Second,  // 5초 타임아웃
			PermitWithoutStream: false,            // 활성 RPC 없으면 핑 비활성화
		}),
	)
	if err != nil {
